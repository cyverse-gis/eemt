#!/usr/bin/env python3
"""
Download DAYMET v4 R1 daily data for SE Arizona study area.

Downloads tmin, tmax, prcp, vp for specified years via OPeNDAP
from NASA Earthdata (requires ~/.netrc credentials).

Usage:
    python scripts/download_seaz_daymet.py
    python scripts/download_seaz_daymet.py --start-year 1980 --end-year 2024
    python scripts/download_seaz_daymet.py --year 2020
"""
...
# SE Arizona bounding box (WGS84) covering:
# Santa Catalina, Tortolita, Tucson, Rincon, Santa Rita Mountains + Tucson
SEAZ_BBOX = {
    "north": 32.55,
    "south": 31.65,
    "east": -110.40,
...
BBOX_BUFFER = 0.02  # degrees buffer for full DAYMET pixel coverage
...
OUTPUT_BASE = "/opt/tswetnam/data/seaz/daymet/daily"
...
def lcc_to_grid_indices(lcc_bbox, buffer_pixels=2):
    """Convert LCC coordinates to DAYMET grid indices."""
    g = DAYMET_GRID
...
def opendap_download(variable, year, indices, output_path):
    """Download via OPeNDAP using curl with Earthdata auth."""
...
def main():
    parser = argparse.ArgumentParser(description="Download DAYMET for SE Arizona")
    ...
    print("DAYMET Download — SE Arizona")
    print("=" * 60)
    print(f"Bbox: W={SEAZ_BBOX['west']} S={SEAZ_BBOX['south']} "
          f"E={SEAZ_BBOX['east']} N={SEAZ_BBOX['north']}")
    print(f"Years: {years[0]}-{years[-1]} ({len(years)} years)")
    print(f"Variables: {', '.join(DAYMET_VARS)}")

    # Compute grid indices
    print("\nComputing DAYMET grid indices...")
    lcc_bbox = bbox_to_lcc(SEAZ_BBOX)
    ...
    for year in years:
        for var in DAYMET_VARS:
            out_path = os.path.join(args.output_dir, var, f"{var}_{year}_seaz.nc")
