#!/usr/bin/env python3
"""
Container wrapper for EEMT Full Workflow
Executes eemt/run-workflow within the Docker container environment
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run Full EEMT Workflow in Container')
    ...
    parser.add_argument('--start-year', type=int, default=2020, help='Start year for climate data')
    parser.add_argument('--end-year', type=int, default=2020, help='End year for climate data')
    ...
    logger.info(f"Starting full EEMT workflow for job {args.job_id}")
    logger.info(f"DEM: {args.dem}")
    logger.info(f"Output: {args.output}")
    logger.info(f"Climate period: {args.start_year}-{args.end_year}")
    ...
    work_dir = Path("/tmp") / f"eemt_work_{args.job_id}"
    ...
    # Build command for original eemt/run-workflow
    cmd = [
        'python', '/opt/eemt/eemt/eemt/run-workflow',
    ...
    try:
        ...
        if return_code == 0:
            logger.info(f"EEMT workflow completed successfully for job {args.job_id}")
            print("workflow completed successfully")
        else:
            logger.error(f"EEMT workflow failed with return code {return_code}")
            return return_code
            
    except Exception as e:
        logger.error(f"Error executing EEMT workflow: {e}")
        return 1
